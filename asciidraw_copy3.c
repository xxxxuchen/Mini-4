...
int main(){
	...
	while (exit != 0){
		...
		if (strcmp(command,"END") == 0){
        	...
        	int x1, y1, x2, y2;
			sscanf(input,"%s %d,%d %d,%d", garbage,&x1,&y1,&x2,&y2);
			if (x2 < x1 ){ // Avoid negative number; aka x1 and y1 are always the smaller values
				int temp = x1;
				x1 = x2;
				x2 = temp;
			}
			...
			if (x2 - x1 == 0){     // vertical line
				...
				for (int j = x1; j <= x2; j++){ // drawing from left to right 
					...
					for (int i = yprevious; i <= rdvalue; i++){ // drawing from bottom to top
						grid[row-i-1][j] = cha;
					}
					yprevious = rdvalue;
				}
			...
			while(xcurrent != xend || ycurrent != y_center){
				double diff = 100000;
